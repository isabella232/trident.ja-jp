...
.ONTAP NASエコノミーの最小構成例
[%collapsible%open]
====
これは、 `ontap-nas-economy` ドライバ。
...
----
====
.ONTAP NAS FlexGroupの最小構成例
[%collapsible%open]
====
これは、 `ontap-nas-flexgroup` ドライバ。
...
[%collapsible%open]
====
この例では、動的なエクスポートポリシーを使用してエクスポートポリシーを自動的に作成および管理するようにAstra Tridentに指示します。これは、でも同様に機能します `ontap-nas-economy` および `ontap-nas-flexgroup` ドライバ。
...
[%collapsible%open]
====
この例では、SMBボリュームを使用してAmazon FSx for ONTAPバックエンドを作成します。
