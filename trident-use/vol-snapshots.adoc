...
. ボリュームのSnapshot作成
+
Kubernetes 1.20以降では、v5.0以上のスナップショットコンポーネントでv1スナップショットCRDを使用します。Kubernetesバージョン1.17-1.19の場合は、v1beta1にv3.0.3のスナップショットコンポーネントを使用します。
...
+
Kubernetes 1.20以降ではv5.0以上を使用してください。Kubernetesバージョン1.17~1.19では、v3.0.3を使用します
