...
[role="lead"]
を使用して、Astra Tridentをインストールできます `tridentctl`。このプロセスでは、TridentイメージとCSIイメージがにある環境 のインストールを行います `docker.io` および `registry.k8s.io` または、プライベートのミラー化されたレジストリ。をカスタマイズします `tridentctl` 配置については、を参照してください link:kubernetes-customize-deploy-tridentctl.html["tridentctl 展開をカスタマイズします"]。
...
----
--
.1つのミラー化されたレジストリにTridentイメージとCSIイメージが含まれます
--
[listing]
----
...
----
--
.TridentとCSIのイメージが、異なるミラーレジストリに含まれています
--
