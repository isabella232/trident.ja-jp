...
インチ `operator.yaml`、を更新します `image: docker.io/netapp/trident-operator:23.01.0` イメージレジストリの場所を反映します。例：

* `image: <your-registry>/trident-operator:23.01.0` すべての画像が1つのレジストリにある場合。
* `image: <your-registry>/netapp/trident-operator:23.01.0` TridentイメージとCSIイメージが異なるレジストリにある場合。
...
=== 手順4：でイメージのレジストリの場所を更新します `TridentOrchestrator`

TridentイメージとCSIイメージは1つのレジストリまたは異なるレジストリに配置できますが、CSIイメージはすべて同じレジストリに配置する必要があります。更新 `deploy/crds/tridentorchestrator_cr.yaml` レジストリ設定に基づいて追加の場所の仕様を追加します。

[role="tabbed-block"]
====
.TridentとCSIのイメージを1つのレジストリに格納します
--
[listing]
----
...
----
--
.TridentとCSIのイメージが異なるレジストリにあります
--
...
=== 手順5：を作成します `TridentOrchestrator` Tridentをインストール
