...
クラスタを対象としたTridentオペレータのインストールから、クラスタを対象とした別のTridentオペレータのインストールにアップグレードできます。すべてのAstra Tridentバージョン21.01以降では、クラスタを対象とした演算子を使用します。
...
. を使用します `helm upgrade` コマンドを入力します `trident-operator-23.07.0.tgz` アップグレード後のバージョンが反映されます。
+
[listing]
----
